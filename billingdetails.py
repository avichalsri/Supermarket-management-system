...
def billingitems():
    ...
    Label(billingsto,text='-'*54+'Billing'+'-'*54,font=("Courier", 10)).grid(row=0,column=0,columnspan=7,sticky='W')
    Label(billingsto,text='Enter Name: ',font=("Courier", 10)).grid(row=1,column=0)
    name1=Entry(billingsto,width=20)
    name1.grid(row=1, column=1)
    Label(billingsto,text='Enter Address: ',font=("Courier", 10)).grid(row=2,column=0)
    add=Entry(billingsto,width=20)
    add.grid(row=2, column=1)
    
    Label(billingsto,text='-'*115,font=("Courier", 10)).grid(row=6, column=0,columnspan=7,sticky='W')
    Label(billingsto,text='Select Item',relief='ridge',width=15,font=("Courier", 10)).grid(row=7,column=0)
    Label(billingsto,text='Qty_Remain',relief='ridge',width=12,font=("Courier", 10)).grid(row=7,column=1)
    Label(billingsto,text='Cost',relief='ridge',width=5,font=("Courier", 10)).grid(row=7,column=2)
    Label(billingsto,text='Expiry Date',width=15,relief='ridge',font=("Courier", 10)).grid(row=7,column=3)
   
    Button(billingsto,text='Add to bill',width=15,command=addtothebill,font=("Courier", 10)).grid(row=8, column=6)
    Label(billingsto,text='Quantity',width=20,relief='ridge',font=("Courier", 10)).grid(row=7, column=5)
    qtys=Entry(billingsto)
    qtys.grid(row=8,column=5)
    refresh()
    Button(billingsto,width=15,text='Main Menu', command= mainmenu,font=("Courier", 10)).grid(row=1,column=6)
    Button(billingsto,width=15,text='Refresh Stock',command=refresh,font=("Courier", 10)).grid(row=3,column=6)
    Button(billingsto,width=15,text='Reset Bill',command=resetbill,font=("Courier", 10)).grid(row=4,column=6)
    Button(billingsto,width=15,text='Print Bill',command=printbill,font=("Courier", 10)).grid(row=5,column=6)
    Button(billingsto,width=15,text='Save Bill',command=savebill,font=("Courier", 10)).grid(row=7,column=6)
...
def savebill():
    ...
    ntotal=round(total)
    lineadd='\n\n\n'
    lineadd+="         Supermarket Management System\n"
    lineadd+="   NIT Manipur, Langol, Imphal West, Manipur\n\n"
    lineadd+="===============================================\n"
    lineadd+="                                  Bill No: %s\n" % details[5]
    ...
    for i in range(len(sl)):
        if names[i] != 'nil':
            ...
            lineadd+=s1
    lineadd+="\n-----------------------------------------------\n"
    lineadd+='Total'+(' '*24)+(' '*(12-len(str(total)))) +'Rs. '+ str(total)+'\n'
    details[3]=str(total)
        
    lineadd+="-----------------------------------------------\n\n"
    lineadd+="                           Avichal Srivastava\n"
    lineadd+="                                Signature \n"
    lineadd+="===============================================\n"
    print(lineadd)
    p=time.localtime()
...
def dailyincome():
    ...
    flag='dailyinco'
    dailyinco=Tk()
    dailyinco.title('INCOME')
    dailyinco.geometry("300x235")
    dailyinco.wm_iconbitmap('favicon.ico')
    total=0.0
    today=str(time.localtime()[2])+'/'+str(time.localtime()[1])+'/'+str(time.localtime()[0])
    Label(dailyinco,text='Date: '+today,font=("Courier", 10)).grid(row=0,column=0)
    cur.execute('select * from bill')
    for i in cur:
        if i[4]==today:
            total+=float(i[3])
    
    Label(dailyinco,width=34,text="Today's Total Income: Rs. "+str(total), bg='black',fg='white',font=("Courier", 10)).grid(row=1,column=0)
    index=0
    vsb=Scrollbar(orient='vertical')
    lb1=Listbox(dailyinco,width=25, yscrollcommand=vsb.set)
    vsb.grid(row=3,column=1,sticky=N+S)
    ...
    c.commit()
    Button(dailyinco,text='Main Menu',width=10,command=mainmenu,font=("Courier", 10)).grid(row=16,column=0)
    dailyinco.mainloop()
